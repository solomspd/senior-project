...
class cavaj(nn.Module):
	...
	def __init__(self, arg) -> None:
		...
		self.llc_embed = nn.Embedding(206,124)
		self.ast_embed = nn.Embedding(self.EOS_TOK+2,64)
		self.device = arg.device
		self.max_len = arg.ast_max_len

	def forward(self, llc):
		llc = llc.to(self.device)
		llc.x = self.llc_embed(llc.x.T.squeeze())
		...
		while not stop and i < self.max_len:
			ast_embedded = ast.clone().detach().to(self.device)
			ast_embedded.x = torch.argmax(ast_embedded.x.T,0)
			ast_embedded.x = self.ast_embed(ast_embedded.x)
			dec_out = self.dec(ast_embedded, enc_out)
			new_node = Data(self.new_node_final(dec_out.x), dec_out.edge_index)
			new_node.x = pyg.global_add_pool(new_node.x, batch=None) # collapse output of variable size to a single 1
...
class encoder(nn.Module):
	...
	def __init__(self, arg) -> None:
		super().__init__()
		self.src_embed = pyg.SAGEConv(-1, arg.hid_dim)
